...
A journal represents a summary of spending by consumers over a span of time.
It is represented as a charge container.  A journal can be marked as closed, at
which time new transfers will be recorded on a new journal.  Money consumed by
a consumer is transferred I<to> the journal.
...
a transfer is made from the consumer to the journal.  If the consumer
has no funds, the consumer is responsible for issuing appropriate warnings to
the user and eventually for cutting off service.

A service such as Listbox posting can ask a consumer if it is sufficiently
well-funded, and can deny service if not.
...
an invoice.  The charge, when paid, will have its value transferred to the
consumer that placed the charge.
...
and are automatically filed by consumers, transferring funds from the consumer
to the journal.  These are not sent to customers on invoices.  They are a
purely internal means of accounting how the customer's payments were eventually
...
Each transfer has a source object (typically a consumer), a target object
...
transfer.  For example, the normal transfer of money from a consumer to a
journal that occurs when the consumer enters a charge is has type
...
Coupons are attached to consumers.  When ever the consumer is about to
charge either the invoice or the ledger, it hands the charge arguments
to the coupon first. The coupon has the option of modifying the
arguments before the charge is created.  A 25%-off coupon will adjust
...
Because one of the invoice charges composes the "Bankable" role, its value is
transferred from the invoice to the consumer.
...
The method first scans over all the consumers looking for "spare change":
consumers with outstanding funds that are expired.  It transfers all the funds
from these inactive consumers to a fresh credit.
...
sufficiently well-funded, that is all that is needed; the charge is now
in the right cost tree and the later receipt will inform the customer.
