...
number of banks, consumers, journals, and invoices.  It also singles out
the currently-active banks and consumers, the current "general fund"
bank, if there is one, and the current invoice.  It also
...
transfers as needed to pay for its continued operation.  A bank may be a
"general fund", not associated with any consumer but available to any consumer
whose associated bank is insufficient to cover its demands.
...
If the bank is not well-enough funded, the account will check to
see if there is a general fund bank from which it can draw.
