...
for my $test (
  [ 'one-off charges'   => { consumes_funds => 1 } =>   0 ],
  [ 'slush-fund charge' => {                     } => 100 ],
) {
  my ($desc, $c_arg, $left) = @$test;
  ...
  test $desc => sub {
    ...
    Moonpig->env->storage->do_with_ledger($guid, sub {
      ...
      is(
        !! $consumer->consumes_funds,
        !! $c_arg->{consumes_funds},
        "consumes_funds was set"
      );
      ...
      is($ledger->amount_available, dollars($left), "amount available: \$$left");
    });
  };
}
